                    ...
                    usersRef.orderByChild("current_floor").equalTo(lastCurrency+i).once("value")
                        .then(function(snapshot){
                            ...
                            for (var i=0; i<usersKey.length;i++) {
                                var user = users[usersKey[i]];
                                var msg = {
                                    last_floor: user["current_floor"],
                                    updated_floor: Number(currency)
                                }
                                user["current_floor"] = Number(currency);
                                fcmPush(user["fcm_key"], msg);
                            }
